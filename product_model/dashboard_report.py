# Model dashboard for validation analysis (PRODUCT MODEL)
import pandas as pd
...
def dashboard_report(val_csv_path=None, output_dir=None):
	"""
	Generate dashboard metrics and mismatches for validation predictions (PRODUCT MODEL).
	Args:
	...
	print("\n==== Metrics for PRODUCT_MODEL Validation Set ====")
	print("\nClassification Report:")
	print(classification_report(y_true, y_pred, digits=4, target_names=['CG', 'OR']))
	print("Confusion Matrix:")
	print(confusion_matrix(y_true, y_pred, labels=['CG', 'OR']))
	print(f"Accuracy: {accuracy_score(y_true, y_pred):.4f}\n")

	return {
		'classification_report': classification_report(y_true, y_pred, digits=4, target_names=['CG', 'OR'], output_dict=True),
		'confusion_matrix': confusion_matrix(y_true, y_pred, labels=['CG', 'OR']).tolist(),
